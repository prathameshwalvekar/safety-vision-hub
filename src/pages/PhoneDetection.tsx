...
const PhoneWebcamSection = () => {
  ...
  const stopWebcam = () => {
    ...
    setIsStreaming(false); setIsDetecting(false); setDetections([]); setStats({ phone: 0, remote: 0 });
  };
  ...
  const drawDetections = useCallback((data: any) => {
    ...
    data.detections?.forEach((d: any) => {
      ...
      const color = d.label === 'cell phone' ? '#ff9800' : '#2196f3';
    ...
    ctx.fillText(`📱 ${data.phone_count || 0}  📺 ${data.remote_count || 0}`, 10, 25);
  }, []);
  ...
  const runDetection = useCallback(async () => {
    ...
    try {
      ...
      setDetections(result.detections || []); setStats({ phone: result.phone_count || 0, remote: result.remote_count || 0 });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-1 gap-3">
          <div className="detection-stat-card"><span className="stat-icon">📱</span><span className="stat-value">{stats.phone}</span><span className="stat-label">Phones</span></div>
          <div className="detection-stat-card"><span className="stat-icon">📺</span><span className="stat-value">{stats.remote}</span><span className="stat-label">Remotes</span></div>
        </div>
      </div>
      {detections.length > 0 && (
        <div className="card-surface p-4">
          ...
          <div className="space-y-2">
            {detections.map((d: any, i: number) => (
              <div key={i} className="detection-item-row"><span className="label">{d.label === 'cell phone' ? '📱' : '📺'} {d.label}</span><span className="confidence">{(d.confidence * 100).toFixed(1)}%</span></div>
            ))}
          </div>
        </div>
      )}
